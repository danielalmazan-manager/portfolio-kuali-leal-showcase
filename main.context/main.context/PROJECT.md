...
7. ✅ **Sistema de Onboarding Progresivo** → SSO ligero, rutas exactas, pasos opcionales (2026-03-27).
8. ✅ **Fix Botón "Mi Negocio"** → Redirección cross-domain con URLs absolutas (2026-03-27).
9. ✅ **SessionPayload Extendido** → Campos de onboarding en JWT compartido (2026-03-27).
...
8. Todo cambio de schema requiere script de migración en `src/scripts/` o `migrations/`.
...
11. **Cross-domain redirects:** SIEMPRE usar URLs absolutas (ej: `redirect('https://app.kualileal.com/...')`).
12. **Onboarding:** Usar rutas EXACTAS, NO genéricas como `/onboarding`. Ver `src/types/onboarding.ts`.
13. **SessionPayload:** Incluye campos de onboarding (`has_business_module_active`, etc.). Ver `src/lib/auth.ts`.
14. **Cookies SSO:** Configuradas con `domain: ".kualileal.com"` en producción para compartir sesión.
